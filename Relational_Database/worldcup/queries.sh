...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams FULL JOIN games as w ON teams.team_id = w.winner_id FULL JOIN games as op ON teams.team_id = op.opponent_id WHERE (w.year = 2014 AND w.round = 'Eighth-Final') OR (op.year = 2014 AND op.round = 'Eighth-Final') ORDER BY name")"
